/* XS 16.2.2 RECOVERY — restored V16.2 interaction engine; cache-bust */
(() => {
  "use strict";
  ...
  function fromB64url(text) { const base = text.replace(/-/g, "+").replace(/_/g, "/"); const padded = base + "=".repeat((4 - (base.length % 4)) % 4); return b64ToBytes(padded); }
  function ownerSecret() { try { return JSON.parse(storageGet(OWNER_SECRET_STORE) || "null"); } catch { return null; } }
  async function derivePassword(password, salt, iterations = 180000) {
  ...
  async function setOwnerPassword(password) {
    ...
    storageSet(OWNER_SECRET_STORE, JSON.stringify({ salt: bytesToB64(salt), hash: bytesToB64(hash), iterations: 180000 }));
  }
  async function verifyOwnerPassword(password) {
    const secret = ownerSecret(); if (!secret) return false;
    const hash = await derivePassword(password, b64ToBytes(secret.salt), secret.iterations || 180000);
    return bytesToB64(hash) === secret.hash;
  }
  function hasOwnerPassword() { return !!ownerSecret(); }
  function passkeyInfo() { try { return JSON.parse(storageGet(OWNER_PASSKEY_STORE) || "null"); } catch { return null; } }
  function hasPasskey() { return !!passkeyInfo()?.id; }
  async function enrollPasskey() {
    if (!window.PublicKeyCredential || !navigator.credentials) throw new Error("Passkey não suportada neste navegador");
    const challenge = crypto.getRandomValues(new Uint8Array(32));
    const userId = crypto.getRandomValues(new Uint8Array(16));
    const credential = await navigator.credentials.create({ publicKey: {
      challenge,
      rp: { name: "XS Manager" },
      user: { id: userId, name: `xs-admin-${location.hostname}`, displayName: "XS Administrator" },
    ...
    storageSet(OWNER_PASSKEY_STORE, JSON.stringify({ id: b64url(new Uint8Array(credential.rawId)), createdAt: Date.now() }));
    return true;
  }
  async function authenticatePasskey() {
    const info = passkeyInfo(); if (!info) return false;
    if (!window.PublicKeyCredential || !navigator.credentials) throw new Error("Passkey não suportada neste navegador");
  ...
  function updateAdminSessionStatus() {
    ...
    if (!ownerAuthenticated) {
      pill.className = "admin-session-pill offline";
    ...
    if (remaining <= 0) {
      pill.className = "admin-session-pill warning";
      value.textContent = "Sessão expirada";
      if (remoteAdminExpiresAt) setTimeout(() => lockAdmin("Sessão Admin expirada"), 0);
      return;
    }

    pill.className = `admin-session-pill ${remaining <= 300000 ? "warning" : "active"}`;
    value.textContent = `Sessão ${fmtCountdown(remaining)}`;
    pill.title = `Tempo restante da sessão no servidor. Bloqueio por inatividade: ${Math.max(1, Number(config.owner.sessionMinutes || 30))} min.`;
  }
  ...
  function scheduleManagerReauth() {
    ...
    managerResumeTimer = setTimeout(() => {
      // Só encerra a retomada se o Manager continuar minimizado.
  ...
  function canQuickResumeManager() {
    if (!managerSessionStillValid()) return false;
    if (!managerResumeUntil) return true; // sessão recém autenticada / Manager ainda ativo
    return Date.now() <= managerResumeUntil;
  }
  ...
  function minimizeManagerToHome() {
    ...
    // Reconstrói apenas a Home pública e a navegação.
    renderIdentity();
    buildNav();
    renderHome();

    // Uma única fonte de verdade para trocar a tela.
    openView("home");
  ...
  async function enterManagerFromKey() {
    // Reabertura imediata, sem autenticação repetida, por até 5 min fora do Manager.
    if (managerSessionStillValid()) {
    ...
    // Tenta restaurar uma sessão remota ainda válida (ex.: refresh da página).
  ...
  function lockAdmin(message = "Admin bloqueado") {
    ...
    remoteAdminSession = ""; sessionStorage.removeItem("xs-v13-admin-session"); setRemoteAdminExpiry(0); clearInterval(adminSessionStatusTimer);
  ...
  function toast(msg, type = "ok") { const t = $("#toast"); t.textContent = msg; t.className = `toast ${type}`; clearTimeout(toast._t); toast._t = setTimeout(() => t.classList.add("hidden"), 2200); }
  function showAdminModal({ title, text = "", confirmText = "Confirmar", danger = false, fields = [] }) {
    return new Promise(resolve => {
      const dialog = $("#adminModal"), form = $("#adminModalForm"), wrap = $("#adminModalFields"), confirmBtn = $("#adminModalConfirm");
      $("#adminModalTitle").textContent = title; $("#adminModalText").textContent = text; confirmBtn.textContent = confirmText;
      ...
      dialog.addEventListener("close", done, { once: true }); dialog.showModal(); setTimeout(() => $("#adminModalField0")?.focus(), 40);
    });
  }
  ...
  function applyTheme(source = config) {
    ...
    r.style.setProperty("--accent", a.accent || "#ff63d8");
    r.style.setProperty("--accent2", a.accent2 || "#6ddcff");
    ...
    $("#overlay").style.background = `rgba(${hexRgb(a.overlayColor || "#050712")},${Number(a.overlayOpacity ?? .44)})`;
    $("#ambient").classList.toggle("off", a.ambientGlow === false);
    $("#ambient").classList.toggle("no-particles", a.particles === false);
    $("#cursorGlow").classList.toggle("on", !!m.cursorGlow && !m.reducedMotion);
  ...
  function applyLockTheme(source = config) {
    ...
    $("#lockTitle").textContent = l.title || "Conecte seu padrão";
    $("#lockSubtitle").textContent = l.subtitle || "Desenhe a sequência para acessar.";
    $("#lockKicker").textContent = l.kicker || "PRIVATE SPACE";
    $("#lockKicker").style.display = l.showKicker === false ? "none" : "";
    $("#clearPattern").style.display = l.showClear === false ? "none" : "";
  }

  function renderIdentity() {
    const p = config.profile;
    $("#topRegname").textContent = `@${p.regname || "user"}`;
    $("#topUid").textContent = `UID ${p.uid || "—"}`;
    const av = $("#topAvatar");
    av.style.backgroundImage = p.avatar ? `url("${String(p.avatar).replace(/"/g, '\\"')}")` : "";
  }
  ...
  function renderHome() {
    ...
    $("#hero").dataset.homeLayout = h.layout || "center";
    $("#homeModules").innerHTML = parts.join("");
  }
  ...
  function logout() { audio.pause(); lockAdmin(""); app.classList.remove("unlocked"); app.classList.add("locked"); document.body.classList.remove("focus-mode"); openView("home"); mainPattern.reset(); hideOwnerGate(); $("#notifPanel").classList.add("hidden"); $("#lightbox").classList.add("hidden"); toast("Perfil bloqueado"); }
  ...
  function createPatternLock({ root, canvas, dotSelector, target, minPoints, feedback, onSuccess, onError, remoteValidate }) {
    ...
    function begin(e, dot) {
      ...
      // Captura no próprio ponto quando suportado, mas a continuidade
      // não depende disso porque move/up são observados no document.
    ...
    // O gesto começa SOMENTE em um ponto.
    dots.forEach(dot => {
    ...
    // Continuação capturada globalmente: não perde o gesto ao sair do botão/root.
  ...
  const mainPattern = createPatternLock({
    root: $("#pattern"), canvas: $("#patternCanvas"), dotSelector: "[data-dot]", target: () => config.access.pattern || [], minPoints: () => config.access.minPoints || 4, feedback: $("#patternFeedback"),
    onSuccess: () => { attempts = 0; setTimeout(unlock, 250); },
    onError: () => { attempts++; if (attempts >= (config.access.maxAttempts || 5)) { lockedUntil = Date.now() + (config.access.lockSeconds || 15) * 1000; attempts = 0; countdownLock(); } }
  });

  function countdownLock() {
    const el = $("#patternFeedback"); const tick = () => { const left = Math.max(0, Math.ceil((lockedUntil - Date.now()) / 1000)); if (!left) return mainPattern.reset("Tente novamente"); el.textContent = `Aguarde ${left}s`; el.className = "feedback error"; setTimeout(tick, 500); }; tick();
  }
  ...
  function startOtpCountdown(seconds = 30) {
    ...
    otpCountdownTimer = setInterval(() => {
      ...
      if (left <= 0) {
        ...
        $("#otpStatus").textContent = "Código expirado. Volte e solicite um novo código com sua chave.";
  ...
  async function refreshAuthHealth() {
    ...
    try {
      ...
      else if (!data.accessKeyConfigured && (!data.resendConfigured || !data.emailConfigured)) el.textContent = "Configure ADMIN_ACCESS_KEY, RESEND_API_KEY e ADMIN_EMAIL no Render.";
      else if (!data.accessKeyConfigured) el.textContent = "Configure ADMIN_ACCESS_KEY no Render.";
      else if (!data.resendConfigured || !data.emailConfigured) el.textContent = "Configure RESEND_API_KEY e ADMIN_EMAIL no Render.";
  ...
  async function showOwnerGate() {
    const gate = $("#ownerGate"); gate.classList.remove("hidden"); gate.setAttribute("aria-hidden", "false");
    updateOwnerGateAvailability();
    if (remoteOtpEnabled() && remoteAdminSession) {
    ...
    $("#adminKeyStatus").textContent = ""; $("#otpStatus").textContent = "Aguardando o código…";
    selectOwnerAuth("key"); refreshAuthHealth();
  }
  function hideOwnerGate() {
    const gate = $("#ownerGate"); gate.classList.add("hidden"); gate.setAttribute("aria-hidden", "true");
    clearInterval(otpCountdownTimer); remoteChallengeToken = "";
    const key = $("#adminAccessKey"), input = $("#otpInput");
    if (key) key.value = "";
  ...
  $("#toggleAdminKey")?.addEventListener("click", () => {
    const input = $("#adminAccessKey"); input.type = input.type === "password" ? "text" : "password"; input.focus();
  });
  ...
  async function submitAdminKey() {
    ...
    if (key.length < 4) { status.textContent = "Digite sua chave de acesso."; status.className = "owner-inline-status error"; input.focus(); return; }
    adminAccessInFlight = true; input.disabled = true; btn.disabled = true; btn.textContent = "Autenticando…"; status.textContent = "Validando chave e enviando código…"; status.className = "owner-inline-status";
    try {
      const data = await authRequest("/api/admin/access", { body: { key } });
      remoteChallengeToken = data.challengeToken;
      input.value = "";
      const otp = $("#otpInput"); otp.value = ""; otp.disabled = false; otp.classList.remove("is-error", "is-success", "is-verifying");
      $("#otpAttemptsHint").textContent = "2 tentativas";
      $("#otpStatus").textContent = "Código enviado. Digite os 6 dígitos para entrar.";
      ...
      if (d.locked || d.retryAfter >= 60) { setRemoteLock(d.retryAfter || 600, "Acesso bloqueado temporariamente."); }
        ...
        status.textContent = d.discordRateLimited
          ? (d.retryAfter ? `Não foi possível enviar o e-mail. Verifique o Resend.` : "Não foi possível enviar o e-mail. Verifique o Resend.")
    ...
    } finally { adminAccessInFlight = false; if (!ownerLocked()) { btn.disabled = false; btn.textContent = "Continuar"; input.disabled = false; } }
  }
  ...
  async function verifyOtpAuto() {
    ...
    const input = $("#otpInput"), code = input.value.replace(/\D/g, "").slice(0, 6); if (code.length !== 6) return;
    otpVerifyInFlight = true; input.disabled = true; input.classList.remove("is-error", "is-success"); input.classList.add("is-verifying"); $("#otpStatus").textContent = "Validando código…";
    try {
      ...
      input.classList.remove("is-verifying"); input.classList.add("is-success"); $("#otpStatus").textContent = "Autenticação concluída.";
      ...
      if (d.locked || d.restartPattern) {
        ...
        remoteChallengeToken = ""; input.classList.add("is-error"); input.disabled = true; $("#otpStatus").textContent = "Código expirado. Retornando para a chave…"; setTimeout(() => selectOwnerAuth("key"), 900);
      } else {
        input.classList.add("is-error"); input.disabled = false; $("#otpAttemptsHint").textContent = `${d.errorsRemaining ?? 1} tentativa restante`; $("#otpStatus").textContent = "Código incorreto.";
  ...
  function renderNotifications() {
    ...
    $("#notificationList").innerHTML = list.length ? list.map(n => `<div class="notice"><b>${esc(n.title || "Aviso")}</b><span>${esc(n.text || "")}</span></div>`).join("") : `<div class="notice"><span>Nenhum aviso.</span></div>`;
    $("#notificationDot").style.display = config.modules.notifications !== false && list.some(n => !n.read) ? "" : "none";
    $("#notificationBtn").style.display = config.modules.notifications !== false ? "" : "none";
  }
  $("#notificationBtn")?.addEventListener("click", () => { $("#notifPanel").classList.toggle("hidden"); config.notifications?.forEach(n => n.read = true); persist(); $("#notificationDot").style.display = "none"; });
  ...
  function renderGallery() {
    ...
    $("#albumFilters").innerHTML = albums.map(a => `<button type="button" class="album-chip ${a === lightboxAlbum ? "active" : ""}" data-album="${esc(a)}">${esc(a)}</button>`).join("");
    $$('[data-album]').forEach(b => b.addEventListener("click", () => { lightboxAlbum = b.dataset.album; renderGallery(); }));
    const items = galleryItems();
    $("#galleryGrid").innerHTML = items.map((g, i) => `<button type="button" class="gallery-item" data-gallery="${i}"><img src="${esc(g.src || "")}" alt="${esc(g.title || `Foto ${i + 1}`)}" loading="lazy"><span>${esc(g.title || `Foto ${i + 1}`)}</span></button>`).join("");
    $$('[data-gallery]').forEach(b => b.addEventListener("click", () => openLightbox(Number(b.dataset.gallery))));
  }
  function openLightbox(i) { lightboxIndex = i; zoom = 1; renderLightbox(); $("#lightbox").classList.remove("hidden"); }
  function renderLightbox() { const items = galleryItems(); if (!items.length) return; lightboxIndex = (lightboxIndex + items.length) % items.length; const g = items[lightboxIndex]; $("#lightboxImg").src = g.src || ""; $("#lightboxImg").style.transform = `scale(${zoom})`; $("#lightboxTitle").textContent = g.title || `Foto ${lightboxIndex + 1}`; $("#lightboxCaption").textContent = g.caption || ""; $("#lightboxCounter").textContent = `${lightboxIndex + 1}/${items.length}`; $("#zoomLabel").textContent = `${Math.round(zoom * 100)}%`; }
  ...
  function renderFriends() { $("#friendsGrid").innerHTML = (config.friends || []).map(f => `<a class="friend-card" href="${esc(f.url || `https://xat.me/${f.regname || ""}`)}" target="_blank" rel="noopener noreferrer"><img src="${esc(f.avatar || "")}" alt=""><div><b>@${esc(f.regname || "Sem nome")}</b><small>UID ${esc(f.uid || "—")}</small></div></a>`).join(""); }
  ...
  function renderLinks() { $("#linksList").innerHTML = (config.links || []).map(l => `<a class="link-card" href="${esc(l.url || "#")}" target="_blank" rel="noopener noreferrer"><span class="link-icon">${esc(linkIcon(l))}</span><b>${esc(l.label || "Link")}</b><span>↗</span></a>`).join(""); }

  function loadTrack(auto = false) {
    const tracks = config.music || [];
    if (!tracks.length) { $("#trackTitle").textContent = "Sem música"; $("#trackArtist").textContent = ""; audio.removeAttribute("src"); return; }
    trackIndex = (trackIndex + tracks.length) % tracks.length;
    const t = tracks[trackIndex];
    $("#trackTitle").textContent = t.title || `Faixa ${trackIndex + 1}`;
    $("#trackArtist").textContent = t.artist || "";
    const cover = $("#trackCover"); cover.textContent = t.cover ? "" : "♫"; cover.style.backgroundImage = t.cover ? `url("${String(t.cover).replace(/"/g, '\\"')}")` : "";
    if (audio.getAttribute("src") !== (t.src || "")) audio.src = t.src || "";
    if (auto && t.src) audio.play().catch(() => {});
  ...
  function updatePlayerState() { $("#playBtn").textContent = audio.paused ? "▶" : "❚❚"; $("#shuffleBtn").classList.toggle("active", !!config.player.shuffle); $("#repeatBtn").classList.toggle("active", config.player.repeat !== "off"); $("#repeatBtn").textContent = config.player.repeat === "one" ? "↻¹" : "↻"; updateNowPlaying(); }
  function renderTrackList() { $("#trackList").innerHTML = (config.music || []).map((t, i) => `<button type="button" class="track-row ${i === trackIndex ? "active" : ""}" data-track="${i}"><div><b>${esc(t.title || `Faixa ${i + 1}`)}</b><small>${esc(t.artist || "")}</small></div><span>${i === trackIndex && !audio.paused ? "♫" : "▶"}</span></button>`).join(""); $$('[data-track]').forEach(r => r.addEventListener("click", () => { trackIndex = Number(r.dataset.track); loadTrack(true); })); }
  function updateNowPlaying() { const t = (config.music || [])[trackIndex]; const b = $("#nowPlaying"); if (!t || audio.paused || $("#view-music").classList.contains("active")) return b.classList.add("hidden"); b.classList.remove("hidden"); b.classList.add("playing"); $("#nowPlayingText").textContent = t.title || "Tocando"; }
  ...
  audio.addEventListener("play", () => { initAudioAnalyser(); updatePlayerState(); drawVisualizer(); }); audio.addEventListener("pause", updatePlayerState);
  ...
  function initAudioAnalyser() {
    if (!config.player.visualizer || analyser) return;
    try { audioCtx = new (window.AudioContext || window.webkitAudioContext)(); sourceNode = audioCtx.createMediaElementSource(audio); analyser = audioCtx.createAnalyser(); analyser.fftSize = 64; sourceNode.connect(analyser); analyser.connect(audioCtx.destination); } catch { analyser = null; }
  }
    ...
    const renderFrame = () => {
      ...
      for (let i = 0; i < bars; i++) {
        const val = data
          ...
          : (audio.paused ? .08 : .22 + Math.abs(Math.sin(Date.now() / 260 + i)) * .24);
  ...
  function accessEditor() {
    ...
    return `<section class="admin-section security-section"><div class="section-title"><span class="eyebrow">SEGURANÇA ADMIN</span><h3>Autenticação do XS Manager</h3><p>A autenticação administrativa usa chave privada e código temporário enviado por e-mail.</p></div>
      ...
      <div class="info-card"><b>Como trocar sua chave:</b> altere <b>ADMIN_ACCESS_KEY</b> em Environment Variables no Render e reinicie o serviço. A chave nunca é gravada neste perfil, no localStorage ou no GitHub.</div>
    </section>
    ...
    <section class="admin-section lock-designer"><div class="section-title"><span class="eyebrow">TELA DE ENTRADA</span><h3>Design do Pattern público</h3><p>Personalize a primeira tela sem alterar o visual do perfil.</p></div>
      ...
      <div class="info-card"><b>Separação de segurança:</b> o Pattern público é apenas a entrada visual do perfil. A autenticação do XS Manager acontece exclusivamente no backend privado do Render.</div>
    </section>`;
  }
  ...
  function bindSettings() {
    $$('[data-path]', $("#settingsContent")).forEach(i => {
    ...
    $$('[data-remove]', $("#settingsContent")).forEach(b => b.addEventListener("click", () => { removePath(draft, b.dataset.remove); renderSettings(); }));
    $$('[data-add]', $("#settingsContent")).forEach(b => b.addEventListener("click", () => { const t = b.dataset.add; if (t === "gallery") draft.gallery.push({ src: "", title: "Nova foto", caption: "", album: "Destaques" }); if (t === "quotes") draft.quotes.push(""); if (t === "music") draft.music.push({ title: "", artist: "", cover: "", src: "" }); if (t === "friends") draft.friends.push({ regname: "", uid: "", avatar: "", url: "" }); if (t === "links") draft.links.push({ label: "", icon: "auto", url: "" }); if (t === "notifications") draft.notifications.push({ title: "Aviso", text: "", read: false }); renderSettings(); }));
    $$('[data-preset]').forEach(b => b.addEventListener("click", () => { const k = b.dataset.preset; draft.appearance.preset = k; if (k !== "custom") Object.assign(draft.appearance, PRESETS[k]); applyTheme(draft); renderSettings(); }));
    $$('[data-interface-theme]').forEach(b => b.addEventListener("click", () => {
      draft.appearance.interfaceTheme = b.dataset.interfaceTheme;
      applyTheme(draft);
      $$('[data-interface-theme]', $("#settingsContent")).forEach(card => {
    ...
    $$('[data-scroll-theme]').forEach(b => b.addEventListener("click", () => $("#interfaceThemeScroller")?.scrollBy({ left: Number(b.dataset.scrollTheme) * 380, behavior: "smooth" })));
    $$('[data-scroll-palette]').forEach(b => b.addEventListener("click", () => $("#paletteScroller")?.scrollBy({ left: Number(b.dataset.scrollPalette) * 300, behavior: "smooth" })));
    ...
    $$('[data-home-enable]').forEach(i => i.addEventListener("change", () => { draft.home.modules[Number(i.dataset.homeEnable)].enabled = i.checked; renderSettings(); const old = config; config = draft; renderHome(); config = old; }));
    $$('[data-home-move]').forEach(b => b.addEventListener("click", () => { const [i, d] = b.dataset.homeMove.split(".").map(Number); moveArray(draft.home.modules, i, i + d); renderSettings(); }));
    $$('[data-mini]').forEach(b => b.addEventListener("click", () => { const i = Number(b.dataset.mini); if (!draft.access.pattern.includes(i)) draft.access.pattern.push(i); b.classList.add("selected"); $("#miniText").textContent = draft.access.pattern.join(" → "); }));
    $("#clearMini")?.addEventListener("click", () => { draft.access.pattern = []; renderSettings(); });
    $$('[data-lock-theme]').forEach(b => b.addEventListener("click", () => { draft.lockAppearance.theme = b.dataset.lockTheme; applyLockTheme(draft); renderSettings(); }));
    bindProfileManager();
  }

  function bindProfileManager() {
    $$('[data-profile]').forEach(b => b.addEventListener("click", () => { const name = b.dataset.profile; if (name === "Principal") { storageRemove(ACTIVE_PROFILE); config = migrate(window.CONFIG_BASE); const saved = storageGet(STORE); if (saved) config = migrate(JSON.parse(saved)); } else { const profiles = loadProfiles(); if (profiles[name]) { config = migrate(profiles[name]); storageSet(ACTIVE_PROFILE, name); } } draft = clone(config); renderAll(); openView("settings"); settingsTab = "profiles"; renderSettings(); toast(`Perfil ${name} carregado`); }));
    $$('[data-delete-profile]').forEach(b => b.addEventListener("click", async e => { e.stopPropagation(); const name = b.dataset.deleteProfile; if (!name) return; const ok = await showAdminModal({ title: `Remover ${name}?`, text: "Esta ação apaga os dados locais desse perfil neste navegador. O perfil Principal nunca é removido.", confirmText: "Remover perfil", danger: true }); if (!ok) return; const profiles = loadProfiles(); delete profiles[name]; saveProfiles(profiles); const active = storageGet(ACTIVE_PROFILE); if (active === name) { storageRemove(ACTIVE_PROFILE); config = loadConfig(); draft = clone(config); renderAll(); } renderSettings(); toast(`Perfil ${name} removido`); }));
    $("#newProfile")?.addEventListener("click", async () => { const values = await showAdminModal({ title: "Novo perfil", text: "Crie uma configuração independente a partir da base padrão.", confirmText: "Criar perfil", fields: [{ label: "Nome do perfil", placeholder: "Ex.: Maria" }] }); const name = values?.[0]?.trim(); if (!name) return; const profiles = loadProfiles(); if (profiles[name] || name === "Principal") return toast("Já existe um perfil com esse nome", "error"); profiles[name] = migrate(window.CONFIG_BASE); profiles[name].profile.regname = name; saveProfiles(profiles); renderSettings(); toast(`Perfil ${name} criado`); });
    $("#duplicateProfile")?.addEventListener("click", async () => { const values = await showAdminModal({ title: "Duplicar perfil", text: "Cria uma cópia completa do perfil atual para você personalizar separadamente.", confirmText: "Duplicar", fields: [{ label: "Nome da cópia", placeholder: "Ex.: Modelo Neon 02" }] }); const name = values?.[0]?.trim(); if (!name) return; const profiles = loadProfiles(); if (profiles[name] || name === "Principal") return toast("Já existe um perfil com esse nome", "error"); profiles[name] = clone(config); saveProfiles(profiles); renderSettings(); toast("Perfil duplicado"); });
    $("#exportProfile")?.addEventListener("click", () => downloadJSON(config, `${config.profile.regname || "perfil"}-xs-v9.json`));
    $("#importProfile")?.addEventListener("change", async e => { const f = e.target.files?.[0]; if (!f) return; try { const incoming = migrate(JSON.parse(await f.text())); config = incoming; draft = clone(config); persist(); renderAll(); renderSettings(); toast("Perfil importado"); } catch { toast("JSON inválido", "error"); } e.target.value = ""; });
  }

  function downloadJSON(data, name) { const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = name; a.click(); URL.revokeObjectURL(url); }
  ...
  $("#saveSettings")?.addEventListener("click", async () => {
    ...
    config = migrate(clone(draft)); persist();
    const active = storageGet(ACTIVE_PROFILE); if (active) { const profiles = loadProfiles(); profiles[active] = clone(config); saveProfiles(profiles); }
    renderAll(); openView("settings"); adminSessionTouch(); updateSettingsSaveState(); toast("Alterações salvas com sucesso");
  });
  $("#resetDraft")?.addEventListener("click", () => { draft = clone(config); applyTheme(config); applyLockTheme(config); renderSettings(); updateSettingsSaveState(); toast("Alterações descartadas"); });
...
$("#settingsTabs")?.addEventListener("click", e => { const b = e.target.closest("[data-tab]"); if (!b) return; settingsTab = b.dataset.tab; renderSettings(); });

  function setPreview(mode) { document.documentElement.dataset.previewMode = mode; ["Auto", "Desktop", "Mobile"].forEach(x => $(`#preview${x}`)?.classList.toggle("active", x.toLowerCase() === mode)); }
  $("#previewAuto")?.addEventListener("click", () => setPreview("auto")); $("#previewDesktop")?.addEventListener("click", () => setPreview("desktop")); $("#previewMobile")?.addEventListener("click", () => setPreview("mobile"));

  $$('[data-close-view]').forEach(b => b.addEventListener("click", closeToHome));
  $("#focusBtn")?.addEventListener("click", () => { if (config.modules.focusMode === false) return; document.body.classList.toggle("focus-mode"); $("#focusBtn").classList.toggle("active", document.body.classList.contains("focus-mode")); });
  ...
  document.addEventListener("visibilitychange", () => { if (document.hidden && ownerAuthenticated && config.owner.lockOnHidden) lockAdmin("Admin bloqueado ao sair da aba"); });
  ...
  document.addEventListener("pointerleave", () => { $("#wallpaper").style.transform = "scale(1.035)"; });

  function renderAll() {
    applyTheme(); applyLockTheme(); renderIdentity(); buildNav(); renderHome(); renderFriends(); renderGallery(); renderLinks(); renderNotifications();
    audio.volume = Number(config.player.volume ?? .7); $("#volume").value = audio.volume; loadTrack(false); restartQuoteTimer(); updateClock();
    $("#focusBtn").style.display = config.modules.focusMode !== false ? "" : "none";
